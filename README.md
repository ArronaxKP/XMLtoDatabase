Content
=======
1. [XMLtoDatabase  - Overview](#xmltodatabase)
2. [How do you use this application](#how-do-you-use-this-application)
3. [Logs & Debugging](#logs-and-debugging)
4. [Mapping XML The basic structure](#mapping-xml-the-basic-structure)
5. [Database Main Insert Database](#database-main-insert-database)
6. [Error Database](#error-database)
7. [Source Database](#source-database)
8. [Example XML](#example-xml)
...
```<type>``` Is the SQL used to cast the object to the correct data type. It will replace the ? in the String with the valuefrom the xpath. e.g. ```CAST ('?' AS DATETIME)``` will be written in the SQL as ```CAST('01 01 01T12345' AS DATETME)``` ```(0..1)```
...
```<name>``` The name of the table (without ```[]```) ```(1)```
...
```<subset>``` To subset the value to a specific set using e.g. TOP 10. This is inserted in the Select statement between SELECT and Column names e.g. ```SELECT ** HERE ** ColumnName FROM TableName``` ```(0..1)```

```<clause>``` The add a clause to the end of the statemtne e.g. WHERE ID = '1'. This is inserted at the end of the select statement e.g. ```SELECT ColumnName FROM TableName ** HERE ** ``` ```(0..1)```
